...
import com.axonivy.utils.axonivycells.demo.exception.WorkbookCreationException;
...
public class CellFactoryBean {
  ...
  public void convert() {
    if (uploadedFile != null) {
      ...
      setConvertedFile(
          buildFileStream(CellFactory.convert().from(uploadedFile.getContent()).toPdf().asBytes(), pdfFileName));
    }
  }
  ...
  public void updateWorkbook() {
    ...
    try {
      ...
      setConvertedFile(buildFileStream(outputStream.toByteArray(), fileName));
    } catch (Exception e) {
  ...
  private String getUpdatedPdfFileName() {
    ...
    if (StringUtils.isNotBlank(originalName)) {
      String baseName = StringUtils.substringBeforeLast(originalName, DOT);
